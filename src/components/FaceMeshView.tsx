...
const FaceMeshView: React.FC<FaceMeshViewProps> = ({ onHeadPoseUpdate }) => {
  ...
  const onResults = (results: any) => {
    ...
    if (results.multiFaceLandmarks && results.multiFaceLandmarks.length > 0) {
      ...
      if (onHeadPoseUpdate && landmarks && landmarks.length >= 468) {
      ...
      // Simplified mask - only key features
      ctx.save();
      ctx.scale(-1, 1);
      ctx.translate(-canvas.width, 0);
      
      // Draw only essential features with simple lines
      ...
      window.drawConnectors(ctx, landmarks, window.FACEMESH_RIGHT_EYE,
        { color: 'rgba(255, 255, 255, 0.9)', lineWidth: 1.5 });
      ...
      const rightEyeLandmarks = [
        ...
        landmarks[374], landmarks[380], landmarks[381], landmarks[382], 
        landmarks[362], landmarks[398], landmarks[384], landmarks[385], 
        landmarks[386], landmarks[387], landmarks[388], landmarks[466]
      ];
      ...
      // Estimate gaze direction from head pose (simplified)
      // Use face center relative to eye center to estimate gaze
      const faceCenter = {
        x: (leftEyeCenter.x + rightEyeCenter.x) / 2,
        y: (leftEyeCenter.y + rightEyeCenter.y) / 2
      };
      
      // Gaze direction based on head orientation (simplified)
      // In real implementation, would use iris position or eye tracking
      const gazeOffsetX = (faceCenter.x - 0.5) * 0.3; // Head yaw affects gaze
      const gazeOffsetY = (faceCenter.y - 0.5) * 0.2; // Head pitch affects gaze
      const gazeLength = 40;
      ...
      // Left eye gaze
      ctx.beginPath();
      ctx.moveTo(leftEyeCenter.x * canvas.width, leftEyeCenter.y * canvas.height);
      ctx.lineTo(
        (leftEyeCenter.x + gazeOffsetX) * canvas.width, 
        (leftEyeCenter.y + gazeOffsetY) * canvas.height
      );
      ctx.stroke();
      
      // Right eye gaze
      ctx.beginPath();
      ctx.moveTo(rightEyeCenter.x * canvas.width, rightEyeCenter.y * canvas.height);
      ctx.lineTo(
        (rightEyeCenter.x + gazeOffsetX) * canvas.width, 
        (rightEyeCenter.y + gazeOffsetY) * canvas.height
      );
      ctx.stroke();
      ...
      ctx.beginPath();
      ctx.arc(leftEyeCenter.x * canvas.width, leftEyeCenter.y * canvas.height, 3, 0, Math.PI * 2);
      ctx.fill();
      ctx.beginPath();
      ctx.arc(rightEyeCenter.x * canvas.width, rightEyeCenter.y * canvas.height, 3, 0, Math.PI * 2);
      ctx.fill();
